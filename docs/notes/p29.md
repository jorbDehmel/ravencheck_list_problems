...
Instead of saying `x = Cons(...)`, we will instead do the thing
that the solver actually does. We begin by expanding the VC.
...
// By construction, y_prime := S(y) always works
```
...
This produces multiple sort cycles. This might not be possible!
